...
*/
    ...
    if (!empty($allDatesInRange)) {
        $missingDataMessage = '<tr class="missing-data"><td colspan="' . (count($columns) + 2) . '" style="color: blue;">Відсутні дані за наступні дати: ' . implode(', ', $allDatesInRange) . '</td></tr>';
    }
    
    if (!empty($weekdays)) {
        $missingWeekdaysMessage = '<tr class="missing-weekdays"><td colspan="' . (count($columns) + 2) . '" style="color: red;">Відсутні дані за БУДНІ наступні дати: ' . implode(', ', $weekdays) . '</td></tr>';
    }
    
    if (!empty($weekends)) {
        $missingWeekendsMessage = '<tr class="missing-weekends"><td colspan="' . (count($columns) + 2) . '" style="color: green;">Відсутні дані за ВИХІДНІ наступні дати: ' . implode(', ', $weekends) . '</td></tr>';
    }
        /*========================================*/
